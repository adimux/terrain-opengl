...
Pos::Pos(double x_f, double y_f, double z_f)
{
	x = x_f;
	y = y_f;
	z = z_f;
...
Pos Pos::difference(Pos autre_position)
{
	return Pos(x - autre_position.x, y - autre_position.y, z - autre_position.z);
}

void Pos::operator=(Pos a_copier)
{
	x = a_copier.x;
	y = a_copier.y;
	z = a_copier.z;
...
Color::Color(int rouge, int vert, int bleu)
{
	r = rouge;
	v = vert;
	b = bleu;
...
Speed::Speed(double distance, Uint32 per_ms)
{
	m_distance = distance;
	m_per_ms = per_ms;
	m_stopped = true;
...
Speed::Speed(const Speed &a_copier)
{
	m_distance = a_copier.m_distance;
	m_per_ms = a_copier.m_per_ms;
	m_stopped = true;
}
...
double Speed::current_speed()
{
	...
	float current_speed = m_distance*( (float)( SDL_GetTicks() - m_last_time)/(float)m_per_ms );
	m_last_time = SDL_GetTicks();
	
	return current_speed;
}

bool Speed::increase_or_not()
{
	if(m_stopped)
		return false;

	if(SDL_GetTicks() - m_last_time >= m_per_ms)
	{
		//cout << SDL_GetTicks() << " " << m_last_time << endl;
		m_last_time = SDL_GetTicks();
...
Dim3::Dim3(float _w, float _l, float _h)
{
	w = _w;
	l = _l;
	h = _h;
...
void Dim3::operator=(Dim3 a_copier)
{
	w = a_copier.w;
	l = a_copier.l;
	h = a_copier.h;	
...
GLuint loadTexture(SDL_Surface *picture_surface, bool useMipMap)
{
    GLuint glID;
